...
import com.mxixm.fastboot.weixin.module.pay.WXPayUtil;
...
public class WxApiExecutor {
    ...
    private Pay payConfig = new Pay();

//    private final ConversionService conversionService;

    public void setPayConfig(Pay payConfig) {
...
//            body = getStringBody(wxApiMethodInfo, args);
            ...
            // 暂时不支持xml转换。。。
            body = getXmlBody(wxApiMethodInfo, args);
    ...
    private Object getXmlBody(WxApiMethodInfo wxApiMethodInfo, Object[] args) {
    	MethodParameter methodParameter = wxApiMethodInfo.getMethodParameters().stream()
                .filter(p -> !BeanUtils.isSimpleValueType(p.getParameterType()) || p.hasParameterAnnotation(WxApiBody.class))
                .findFirst().orElse(null);
        if (methodParameter == null) {
        ...
        WxApiBody wxApiBody = methodParameter.getParameterAnnotation(WxApiBody.class);
        
        
        // 不是简单类型
        Class<?> paramClazz = methodParameter.getParameterType();
        if (!BeanUtils.isSimpleValueType(paramClazz)) {
        	WxApiBody.Scene scene = wxApiBody.scene();
        	Object arg = args[methodParameter.getParameterIndex()];
            String xmlString = null;
            try {
            	if(WxApiBody.Scene.Pay.equals(scene)) { // 支付场景，需要签名
            		WXPayUtil.fillPayParam(arg, payConfig);
            		Map<String, String> bean2MapForXml = WXPayUtil.bean2MapForXml(arg);
            		String key = payConfig.getSecretKey();
            		xmlString = WXPayUtil.generateSignedXml(bean2MapForXml, key , payConfig.getSignType());
            	}else {
            		Map<String, String> bean2MapForXml = WXPayUtil.bean2MapForXml(arg);
            		xmlString = WXPayUtil.mapToXml(bean2MapForXml);
            	}
            	return xmlString;
            }catch(Exception e ) {
            	throw new WxAppException("参数转化出错" , e);
            }
        }
        if (args[methodParameter.getParameterIndex()] != null) {
            return args[methodParameter.getParameterIndex()].toString();
